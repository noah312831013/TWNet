...
def read_zind(partition_path, simplicity_path, data_dir, mode, is_simple=True,
    ...
    for house_index in house_list:
        ...
        for pano in panos:
            ...
            # corners 地板 uv
            if 'vp_aligned_vertices' in layout:
                corner_xz_vp = np.array(layout['vp_aligned_vertices'])
                corner_xz_vp[..., 0] = -corner_xz_vp[..., 0]
                corner_xyz_vp = np.insert(corner_xz_vp, 1, pano['camera_height'], axis=1)
                corners_vp = xyz2uv(corner_xyz_vp).astype(np.float32)
            else:
                corners_vp = None
            ...
            if 'trivial_wall' in pano:
                trivialWalls = np.array(pano['trivial_wall'])
            else:
                trivialWalls = [0]
